...
//
/*
D3BarChart.prototype.getParameters = function (oControlHost) {
		return [ {
			"parameter": oControlHost.configuration["Parameter"],
			
			"values": [ {
				"use" : "Mountaineering Equipment"
				//"use" : this.m_sel.options[this.m_sel.selectedIndex].value
			//"use" : this.m_aData.map(function(d){return d.product})
			}]
		}
		];
	};
...
D3BarChart.prototype.f_onClick = function (oControlHost,paraValue) {
	var oValues = [{'use': paraValue, 'display': paraValue}];
	 ...
	 //var v = oControlHost.page.getControlByName( this.m_sName  ).setValues(paraValue);
	 // var rangeValue = v[0].use;
	 //alert(rangeValue);
	// alert(oControlHost.page.getControlByName( this.m_sName  ).getValues()[0].text);
